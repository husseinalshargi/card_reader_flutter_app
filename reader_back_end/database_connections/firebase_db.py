...
class Firebase_db:
    ...
    # HTTPBearer will take the token securly 
    ...
    def __init__(self):
        try:
            ...
            print('error in firebase db initilization')
            raise HTTPException(400, 'error initilizing firebase db')
        
    #the user_JWS_token is created like this with the "Security" function to handle the header in the request as a dependency 
    # HTTPAuthorizationCredentialsis the output type of the barer
    def get_user_info_from_token(self, user_JWS_token_bearer: http.HTTPAuthorizationCredentials = Security(security_token)) -> dict:
        """
        ...
        """
        # check if the token is correct and enabled, otherwise return an excepetion (rised)
        try:
